<template>
  <div class="addEdit-block">
    ...
    >
      <el-row>
      <el-col :span="12">
        <el-form-item class="input" v-if="type!='info'"  label="敏感字" prop="content">
          <el-input v-model="ruleForm.content"
              placeholder="敏感字" clearable  :readonly="ro.content"></el-input>
        </el-form-item>
        <div v-else>
          <el-form-item class="input" label="敏感字" prop="content">
              <el-input v-model="ruleForm.content"
                placeholder="敏感字" readonly></el-input>
          </el-form-item>
        </div>
      </el-col>
      </el-row>
...
export default {
  data() {
	...
	content : false,
      ...
      rules: {
          content: [
                { required: true, message: '敏感字不能为空', trigger: 'blur' },
          ],
      }
    };
  },
  ...
  methods: {
    ...
    onSubmit() {
      ...
      this.$refs["ruleForm"].validate(valid => {
        if (valid) {
          this.$http({
            url: `mingganzi/${!this.ruleForm.id ? "save" : "update"}`,
            method: "post",
            data: this.ruleForm
          }).then(({ data }) => {
            if (data && data.code === 0) {
              this.$message({
                ...
                onClose: () => {
                  ...
                  this.parent.mingganziCrossAddOrUpdateFlag = false;
                  this.parent.search();
